...
export const getEmpireGambitToken = async (hre:HardhatRuntimeEnvironment) => {
  ...
  return EmpireGambitToken__factory.connect(deploy.address, await getDeployer(hre))
}


task("token:new-minter", "create a new EmpireGambitToken minter and output the new private key")
  .setAction(async(_, hre) => {
    const { ethers } = hre;
    const wallet = ethers.Wallet.createRandom()

    const contract = await getEmpireGambitToken(hre)
    const tx = await contract.grantRole(await contract.MINTER_ROLE(), wallet.address)
    await tx.wait()
...
task("token:fund-minter", "fund a minter account with some sfuel")
  ...
  .setAction(async ({ address }, hre) => {
    ...
    await (await contract.grantRole(await contract.MINTER_ROLE(), address)).wait()
