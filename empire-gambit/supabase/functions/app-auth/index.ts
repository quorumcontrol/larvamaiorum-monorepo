...
serve(async (req) => {
  ...
  const { proof }: { proof: ProofOfRelayer } = await req.json();

  console.log("proof: ", proof);

  // TODO: check exp

  const deploy = deploys[proof.relayer.chainId];
  if (!deploy) {
    return new Response("Invalid chain Id", {
      headers: corsHeaders,
      status: 400,
    });
  }

  const device = ethers.utils.verifyMessage(
    JSON.stringify(proof.relayer),
    proof.signature,
  );
  ...
  const walletDeployer = WalletDeployer__factory.connect(
    walletDeployerAddress,
    provider,
  );

  if (proof.safeDeployed) {
    const safeAddr = await walletDeployer.ownerToSafe(proof.owner);
  ...
  if (!proof.safeDeployed) {
    if (!proof.tokenRequest || !proof.tokenRequestSignature) {
      throw new Error("missing token request or signature")
    }
    const isValid = await authenticateTokenRequest(
      await walletDeployer.STATEMENT(),
      proof.tokenRequest,
      proof.tokenRequestSignature,
    );
